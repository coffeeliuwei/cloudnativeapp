...
> 🎓 **教学/实训入口（强烈推荐先看）**：[**实训指导书·学生版（24 课时）**](docs/00-实训指导书-学生版.md) —— 以本项目为载体，按"本地→云上一一替换"主线，把下面所有章节串成 9 个实训单元 24 课时（环境认知 → 本地跑通 → 云资源 → 平移上云 → EDAS 部署 → CI/CD 流水线 → 域名网络 → 测试运维 → 安全等保），含每单元目标/任务/验收/提交物与综合验收清单。**想系统学"把本地项目上云到生产"，从这里开始。**

| 文档 | 内容 |
|------|------|
| 🎓 [**实训指导书·学生版（24 课时）**](docs/00-实训指导书-学生版.md) | **总教学计划**：9 个实训单元串联全部章节，从本地到生产的完整上云实训，含验收清单 |
