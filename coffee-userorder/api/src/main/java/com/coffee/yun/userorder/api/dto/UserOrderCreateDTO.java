 ...
 *   2. coffee-app 通过 Dubbo RPC 调用 UserOrderInfoService.createOrder(dto)
 *   3. coffee-userorder 将订单写入 MySQL，并发布 "order-created" 消息到 RocketMQ
 *   4. coffee-expresstrack 消费此消息，异步创建快递单和初始轨迹记录
 *
 * 请求体示例：
 *   {
 *     "order_id": "ORDER100",
...
public class UserOrderCreateDTO implements Serializable {
    ...
    /** 订单编号，全局唯一，由调用方（coffee-app）生成并传入（建议使用 UUID 或雪花算法）*/
