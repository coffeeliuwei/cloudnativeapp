 ...
 *
 *   key  →  \xac\xed\x00\x05t\x00\x10order:detail:001
 ...
 *
 * 本配置将序列化方案改为：
 *   key   → StringRedisSerializer   存储为可读字符串，如 "order:detail:001"
 *   value → Jackson2JsonRedisSerializer  存储为 JSON，如 {"order_id":"001",...}
 *
...
public class RedisConfig {
     ...
     *
     * @param factory  Redis 连接工厂，由 spring.data.redis.* 配置自动创建（Spring Boot 自动装配）
     *                 包含连接池、主机地址、端口、密码等信息
    ...
    public RedisTemplate<String, Object> redisTemplate(RedisConnectionFactory factory) {
        ...
        //   - key 必须是 String 类型（如 "order:detail:001"）
        //   - value 可以是任意 Object（如 UserOrderInfoResultDTO 对象）
        ...
        //   存入 Redis 的 JSON：["com.coffee.yun.userorder.api.dto.UserOrderInfoResultDTO",{"order_id":"001",...}]
        ...
        // Spring Data Redis 3.x（Spring Boot 3.x）新 API：
        // 构造函数直接接收 ObjectMapper 参数，替代旧版 setObjectMapper() 方法
