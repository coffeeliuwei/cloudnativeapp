...
本章 8 个 Part 只有一个要动手，其余是"开眼界"：

| 怎么对待 | 哪些 Part | 要做到什么程度 |
|---|---|---|
| **必读 + 动手** | Part 2（域名 + DNS） | 跟着操作，给你的 ECS-3 绑一个域名 |
| **必读** | Part 1（五朵云）、Part 7（决策树） | 看懂，能复述 |
| **扩展阅读** | Part 3-6（VPN / SAG / 专线 / CEN） | **每个方案记住下面那句"一句话"就够**，参数、价格、配置细节将来工作用到再查 |

四个方案的"一句话"提前放在这，读不下去时背这四句也行：

- **VPN 网关**：在公网上挖一条加密隧道连回云上——便宜、几分钟开通，但走公网会抖
- **SAG**：阿里云寄给你一个开箱即用的"接入盒子"（或装个 APP）——适合分支多、不想自己配路由器
- **高速通道**：拉一根物理专线进阿里云机房——贵、要等施工，但延迟个位数毫秒、稳如内网
- **CEN**：把上面所有"航线"汇总调度的"中心机场"——本身不接入，负责接入之后的全网互通
...
### 2.2.1 买域名前先搞清这 5 件事（新手必看，全是商务细节）

> 很多教程只讲"去控制台点几下"，但新手真正卡住的是 **"到底花多少钱、要交什么材料、买完为什么还不能用"** 这些商务问题。这一节专门说清楚，技术操作在 2.3。

**① 域名是"按年租用"，不是一次买断**

你付的是 **使用权年费**，最短买 1 年、最长一次买 10 年。到期必须 **续费**，不续费域名会被释放，别人就能抢注。所以选域名要考虑长期持有成本（看 ②）。

**② 价格：认准"首年价"和"续费价"是两回事（最大的坑）**

阿里云域名价格随促销浮动，下面是 **常见后缀的大致区间**（以阿里云官网实时价为准，这里给数量级让你心里有数）：

| 后缀 | 首年（常有促销） | 续费 / 年（长期成本看这个） | 适合 |
|------|----------------|--------------------------|------|
| `.com` | 约 ¥55 ~ ¥75 | 约 ¥75 ~ ¥85 | 最通用、最保值，**推荐** |
| `.cn` | 约 ¥29 ~ ¥39 | 约 ¥35 ~ ¥39 | 国内项目、便宜 |
| `.xyz` | **低至 ¥9 甚至几块**（促销） | **约 ¥69**（续费贵很多） | 个人练手、预算极低 |
| `.top` / `.vip` / `.site` 等 | 首年几元~十几元 | 续费普遍 ¥30 ~ ¥100 | 临时、练手 |

> ⚠️ **务必看续费价**：很多新后缀"首年 ¥9"很诱人，但 **第二年续费 ¥69**——长期算下来不一定比 `.com` 便宜。**教学练手**用便宜后缀（如 `.xyz` 首年几块）完全够；**真要长期用**直接上 `.com`。
>
> 💡 **一句话预算**：教学走一遍，**首年十几块到几十块**就够；这台 ECS、RDS 按量付费按天几块，域名是其中很小的一笔。

**③ 买域名需要准备的材料（必须实名认证）**

| 你的身份 | 需要的材料 | 说明 |
|---------|----------|------|
| **个人** | 身份证（正反面照片） | 控制台引导上传，提交 **域名实名认证模板** |
| **企业 / 单位** | 营业执照 + 法人/经办人信息 | 单位名义注册，备案也用这套主体信息 |

> 🔥 **`.cn` 域名必须先实名认证通过才能解析使用**（这是工信部规定）；`.com` 等也都要求实名。实名审核一般 **几小时到 1~2 个工作日**。**实名 ≠ 备案**，是两件事，别搞混（实名是给"域名"做的，备案是给"网站"做的，见 ④）。

**④ 中国大陆服务器：买完域名还要"备案"才能用（免费，但要等）**

这是新手最容易懵的一步：**域名买好、解析也配了，浏览器打开却被拦截**——因为大陆地域的 ECS/SAE 上线对外服务，**法律强制要求 ICP 备案**。

| 项目 | 说明 |
|------|------|
| **收费吗** | **免费**（阿里云不收备案费，是合规流程不是商品） |
| **要多久** | 约 **10 ~ 20 个工作日**（含管局审核），急不得 |
| **要什么材料** | 主体材料（个人身份证 / 企业营业执照）、网站负责人信息 + 手机号、**域名证书**、用于承载的**阿里云服务器**（你的 ECS）、按提示用阿里云 APP **拍照/视频核验** |
| **怎么跳过** | 教学想省这 10 天：把 ECS / SAE 地域选 **香港 / 新加坡等海外节点**，海外不需要 ICP 备案，代价是国内访问延迟略高（约 100ms） |

**⑤ 到期与"赎回"——别让域名过期（赎回很贵）**

域名到期不续费不是立刻消失，但 **进入"高价赎回期"**：此时续费要交 **几百元赎回费**（远高于正常续费）。建议在阿里云开 **自动续费**，或到期前邮件/短信提醒时及时手动续。

> **本课程的省钱建议**：练手就买 **便宜后缀首年价**（几块~几十块），或干脆 **用海外地域跳过备案**先把"域名访问"这件事跑通；真正要长期对外的项目，再上 `.com` + 大陆备案。
