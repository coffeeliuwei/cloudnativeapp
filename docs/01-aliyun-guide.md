...
3. 启动时通过环境变量注入地址，`application-prod.yml` 中已预置占位符，无需改代码：

```yaml
# application-prod.yml（main 分支）
dubbo:
  registry:
    address: ${DUBBO_REGISTRY:nacos://mse-xxxxxxxx-p.nacos.mse.aliyuncs.com:8848}
```

部署时在 EDAS / ECS 的环境变量里设置：
```
DUBBO_REGISTRY=nacos://你的MSE地址:8848
NACOS_ADDR=你的MSE地址:8848
DB_HOST=你的RDS地址:3306
DB_USER=数据库用户名
DB_PASSWORD=数据库密码
ENV=prod
```
