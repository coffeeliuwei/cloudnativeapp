...
生产环境有两种使用 Nacos 的方式，**对应本项目的两个不同代码分支**：

---

**方式一：独立使用 MSE Nacos（不接入 EDAS）→ 使用 `main` 分支**

适用场景：只需要服务注册发现，不需要 EDAS 控制台的服务治理功能。使用 **Dubbo 3.x + Spring Boot 3.x**，Nacos 地址由代码中的环境变量控制。

1. 控制台搜索 **MSE** → 创建 Nacos 实例（选择最低规格）
2. 获取内网连接地址，格式类似 `mse-xxx.nacos.mse.aliyuncs.com:8848`
3. 将你的实际地址填入 `application-prod.yml` 的默认值中，然后打包部署：

```yaml
# application-prod.yml（main 分支）——把占位符替换为你的实际地址
dubbo:
  registry:
    address: ${DUBBO_REGISTRY:nacos://你的MSE地址:8848}      # ← 填你的 MSE 内网地址

database:
  user: ${DB_USER:你的RDS用户名}                              # ← 填 RDS 用户名
  password: ${DB_PASSWORD:你的RDS密码}                        # ← 填 RDS 密码
  host: ${DB_HOST:你的RDS地址:3306}                           # ← 填 RDS 内网地址
  dbname: ${DB_NAME:userordertest}

spring:
  config:
    import: "optional:nacos:coffee-userorder.properties?server-addr=${NACOS_ADDR:你的MSE地址:8848}&refreshEnabled=true"
    #                                                                               ↑ 同上，填 MSE 地址
```

> **注意：** `application-prod.yml` 里包含真实密码，**修改后不要 git commit**，只在本地打包用。打包完成后用 `git checkout -- .` 还原。

---

**方式二：通过 EDAS 平台托管（可使用 Dubbo 服务治理）→ 使用 `dubbo2` 分支**

> **为什么 EDAS 要用 dubbo2 分支？**
>
> EDAS 内置的 Nacos（微服务空间绑定的那个）是由 EDAS Agent 自动注入的，**不需要也不能**在代码里写 Nacos 地址——Agent 会帮你接管注册中心连接。
>
> 但这套机制目前与 **Dubbo 3.x** 存在兼容性问题，EDAS Agent 只能稳定支持 **Dubbo 2.x**。因此课程专门创建了 `dubbo2` 分支（Dubbo 2.7.x + Spring Boot 2.7.x）用于 EDAS 部署场景。

`dubbo2` 分支的 `application-prod.yml` **不包含 Nacos 地址配置**，因为 EDAS Agent 自动处理：

```yaml
# application-prod.yml（dubbo2 分支）
# Nacos 注册中心地址由 EDAS Agent 自动注入，此处无需配置
database:
  user: ${DB_USER:userordertest}
  password: ${DB_PASSWORD:Userordertest123}
  host: ${DB_HOST:rm-xxxxxxxxxxxxxxxxx.mysql.rds.aliyuncs.com:3306}
  dbname: ${DB_NAME:userordertest}
```

部署时只需在 EDAS 控制台设置数据库相关环境变量，Nacos 注册由平台自动完成。

详细操作步骤见：[EDAS ECS 集群部署指南](06-edas-deployment.md)

---

**两种方式对比总结：**

| 对比项 | 方式一：MSE Nacos（main 分支）| 方式二：EDAS 托管（dubbo2 分支）|
|--------|------------------------------|-------------------------------|
| 使用分支 | `main`（Dubbo 3.x） | `dubbo2`（Dubbo 2.x） |
| Nacos 地址 | 需要手动配置环境变量 | EDAS Agent 自动注入，无需配置 |
| 服务治理控制台 | 无（只有基础注册发现）| 有（EDAS 控制台可查看 Dubbo 服务）|
| 适合场景 | 只用 MSE，不上 EDAS | 完整使用 EDAS 平台 |
| 费用 | MSE + ECS/RDS | EDAS + ECS/RDS（EDAS 有额外费用）|
...
> **关于 Mirror 配置的注意事项**
>
> 上面的配置把制品库放在 `<profile>` → `<repository>` 里，是**普通仓库**，Maven 会按顺序查找（先本地 `.m2`，再制品库）。
>
> 有些同学会在 `settings.xml` 里额外添加一个 `<mirror>` 来加速下载（比如阿里云公共镜像），如果 `mirrorOf` 设置为 `*`，它会**拦截所有仓库请求**，包括项目内置的 `lib-repo/`。这意味着：
> - 内部 JAR（`coffee-common` 等）必须已经通过 `mvn deploy` 上传到制品库，否则会报 `Could not find artifact`
> - `lib-repo/` 的兜底作用**在此情况下失效**
>
> 如果你配了 mirror 但还没往制品库上传 JAR，**先完成 6.4 节的上传步骤，再构建**。

---

### 6.4 上传 JAR 到私服

> **📌 推荐做法**：下面这些 `mvn deploy` 命令是为了让你 **理解"库怎么进私服、应用怎么拉库"** 的原理。**日常使用本课程推荐交给云效流水线（第 12 章）自动完成**——`git push` 后在网页点几下即可，不用手敲。本节命令属于"第一次/进阶手动跑一遍"，跑通理解后即可转用流水线。
