...
**方式一：独立使用 MSE Nacos（不接入 EDAS）**

适合只需要服务注册发现、不需要 EDAS 控制台治理功能的场景：

1. 控制台搜索 **MSE** → 创建 Nacos 实例（选择最低规格）
2. 获取内网连接地址，格式类似 `mse-xxx.nacos.mse.aliyuncs.com:8848`
3. 在 `application-prod.yml` 中配置：
...
**方式二：通过 EDAS 平台托管（推荐，可使用 Dubbo 服务治理）**

MSE Nacos 绑定到 EDAS 微服务空间后，EDAS Agent 自动接管注册中心连接，无需在代码中填写 Nacos 地址，并可在 EDAS 控制台查看 Dubbo 服务治理信息。
