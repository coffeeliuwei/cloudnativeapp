...
6. [Nacos Config 动态配置详解](#6-nacos-config-动态配置详解)
7. [请求完整调用链路](#7-请求完整调用链路)
8. [模块间依赖关系](#8-模块间依赖关系)
9. [配置分层设计](#9-配置分层设计)
10. [核心设计原则与取舍](#10-核心设计原则与取舍)
...
对于需要频繁调整的参数（如分页上限、业务开关、调优阈值），动态配置可以大幅降低运维成本。
...
order.max-page-size=100     # 最大分页条数
feature.some-flag=false     # 业务开关示例

# coffee-expresstrack.properties
expresstrack.max-page-size=100
```
...
### 6.3 @RefreshScope 工作原理

当你需要某个配置类在 Nacos 变更时自动刷新，可以加上 `@RefreshScope`：

```java
@Component
@RefreshScope       // ← 关键注解：Nacos 配置变更时自动重建此 Bean
public class MyDynamicConfig {

    @Value("${some.config.value:default}")  // 从 Nacos 或本地配置读取
...
3. 点击"+"新建配置：
   ...
   - Group：`DEFAULT_GROUP`
   - 配置格式：`Properties`
   - 配置内容：`order.max-page-size=50`
4. 点击发布
5. 观察 coffee-userorder 控制台，无需重启，配置立即生效
