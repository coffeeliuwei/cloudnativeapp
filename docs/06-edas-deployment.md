...
29. 这一节业务级验证（下单、查快递）等 **Part 8** 部署完前端一起做——**路径 C 的前端同样部署在 ECS-3 的 Nginx 上**（与路径 A/B 完全相同的做法），只是构建时把 API 地址指向本节第 26 步的 **SAE 公网 SLB 域名**，而不是 `ECS-3:8005`。详见 Part 8.1。
...
> - **路径 C(SAE)**:coffee-app 跑在 SAE 上,API 地址改成 Part 7.7 的 **SAE 公网 SLB 域名**
>
...
>
> **为什么不用 OSS？** 阿里云 OSS 静态网站托管确实也是"纯点鼠标"，但它会让前端域名和后端 API 不同源，引出 **CORS 跨域** 这一整套额外知识。用 ECS-3 托管前端，前后端在用户看来同源同 IP，跨域问题天然不存在——所以本课程统一用 ECS-3，不绕 OSS。
...
- **路径 C(SAE)**:`http://<SAE 公网 SLB 域名>`(Part 7.7 第 26 步记下的,无端口号,SLB 默认 80)

1. 用 VSCode 终端进项目根目录的 `app-admin/`:
   ```bash
   cd app-admin
   ```
2. 下面以 **路径 A / B** 的 `http://x.x.x.x:8005` 为例(把 `x.x.x.x` 换成你 **ECS-3 的公网 IP**);**路径 C** 则把整个 `http://x.x.x.x:8005` 换成你的 **SAE 公网 SLB 域名**(无端口号)。
...
3. 进入主页 → 点 **"订单管理 → 订单列表"** → 应能看到订单数据
   ...
   - **路径 C 链路**:浏览器 → **SAE 的 coffee-app**(经 SLB) → **SAE 的 userorder**(Dubbo) → RDS
...
```
                          用户浏览器
                    │                     │
   ① 取前端 http://<ECS-3 IP>:80          │ ③ 前端 JS 把 API 打到 SAE SLB
                    ▼                     ▼
        ┌────────────────────┐   ┌──────────────────────────┐
        │  ECS-3             │   │  SAE 公网 SLB            │
...
> **两张图对比着看**:换路径时 **MySQL / Nacos / Dubbo 调用关系 / 前端在 ECS-3** 这几样完全不变,变的只是"3 个服务跑在哪"(3 台 ECS ↔ SAE 实例)和"coffee-app 的对外入口"(ECS-3:8005 ↔ SAE SLB)。这正是云原生"同一份 jar、换个地方跑"的直观体现。
...
| 后端 API 入口 | localhost:8005 | ECS-3:8005 | SAE 公网 SLB |
| 代码改了几行 | / | **0 行** | **0 行** |
