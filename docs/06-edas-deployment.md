...
14. 同在"高级设置"里找到 **"服务注册发现 → Nacos 注册中心服务发现"** → 选 **`MSE Nacos 专业版`** → 选 Part 3 创建的那个**专业版** MSE 实例。
    ...
    > ⚠️ **`DUBBO_REGISTRY` 还是不用设，但原因要说准**：官方 FAQ 明确说，使用自建或 MSE Nacos 注册中心时 **SAE 不会注入任何环境变量/参数，连哪个注册中心由你的程序自己决定**（自动注入那套只对"SAE 内置 Nacos"生效）。本项目能直接跑通，是因为打包前 `application-prod.yml` 里 `DUBBO_REGISTRY` 的**默认值已经写成了你的 MSE 内网地址**（和 DB 地址同理，见上一条）——所以这里选 MSE Nacos 专业版主要是让 SAE 打通网络与白名单关联，地址本身靠 jar 内默认值。若你 prod.yml 里还是模板的 `mse-xxxxxxxx` 占位地址，这里必须补环境变量 `DUBBO_REGISTRY=nacos://<MSE内网地址>:8848`。
...
23. 进 `coffee-app-sae` 详情 → **"基础信息"** 页的 **"应用访问设置"** 区域（官方文档路径：应用详情页 → 基础信息 → 应用访问设置）
24. 在 **"公网访问地址"** 那一行 → 点 **绑定公网 CLB**
25. 弹窗里：
