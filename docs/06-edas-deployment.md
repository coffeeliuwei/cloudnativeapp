...
16. 点 **"确定"** 保存
    ...
    ![RDS 白名单分组设置弹窗](img/rds-whitelist.png)
    > △ 官方文档截图:把网段从左栏移到右栏(设置),点"确定"
...
10. 在详情页找到 **"接入地址"** 区域,这里有两行地址,分别**复制到记事本里存好**:
    ...
    ![MSE 实例详情页:基础信息与网络信息(内网/公网域名)](img/mse-instance-detail.png)
    > △ 官方文档截图:实例详情"网络信息"区域,红框处即接入地址
...
> 📌 **上传入口不在终端里**:会话管理的网页终端只有命令行。上传文件用 ECS 控制台 **实例详情页** 自带的 **"上传/下载文件"** 功能(同样基于云助手,不需要公网 SSH,单文件最大 500 MB,本项目的 jar 几十 MB 完全够)。入口:ECS 控制台 → 实例列表 → 点实例 ID 进详情页 → 顶部 **"远程命令/文件"** 页签 → **"上传/下载文件"** → **"上传/下载"** 按钮。
>
> ![ECS 实例详情页"远程命令/文件"页签下的上传/下载文件](img/ecs-upload-file-tab.png)
> △ 官方文档截图:橙框处即"远程命令/文件"页签和"上传/下载文件"入口
...
#### 检查上传成功
    ...
    ![ECS 上传文件成功的执行状态列表](img/ecs-upload-file-status.png)
    > △ 官方文档截图:每次上传都会生成一条执行记录,"执行状态"列显示"成功执行"即传到了
...
12. 点 **"确定"**
    ...
    ![EDAS 创建集群弹窗](img/edas-create-cluster.png)
    > △ 官方文档截图:"创建集群"弹窗,集群类型 ECS、网络选专有网络
...
5. 点 **"确定"**
   ...
   > △ 官方文档截图:"创建命名空间"面板,注意底部要选专有网络 VPC
...
11. 第 ① 步 **"应用基本信息"** 填：
    ...
    ![SAE 创建应用第①步"应用基本信息"表单](img/sae-create-app-basic.png)
    > △ 官方文档截图:应用名称、部署方式、资源规格都在第①步这张表单里填
...
14. 同在"高级设置"里找到 **"服务注册发现 → Nacos 注册中心服务发现"** → 选 **`MSE Nacos 专业版`** → 选 Part 3 创建的那个**专业版** MSE 实例。
    ...
    ![SAE 高级设置中的服务注册发现区域](img/sae-registry-mse.png)
    > △ 官方文档截图:三个选项里选中间的"MSE Nacos 专业版",再选实例和命名空间
...
18. 滚到日志最底部，应能看到：
    ...
    ![SAE 实时日志界面](img/sae-logs.png)
    > △ 官方文档截图:实时日志长这样,在这里确认启动信息
...
26. **"确定"** → 等约 1 分钟，会返回一个 **公网 IP / 域名**，**记到记事本**——这就是替代"ECS-3 公网 IP"的新入口。
    ...
    ![SAE 添加公网 CLB 访问弹窗](img/sae-add-clb.png)
    > △ 官方文档截图:"添加公网 CLB 访问"弹窗,选新建 CLB、填监听端口
...
### 8.0 动手前先回答两个问题（决定你在本 Part 怎么走）

本 Part 有两个**互不相干**的分叉,动手前先把自己的答案写下来,后面照着走就不会乱:

**问题①:你后端走的是哪条路径(A / B / C)?——只影响一件事:8.1 构建时填的 API 地址**

| 你的后端路径 | coffee-app 网关跑在哪 | 8.1 构建时 `VUE_APP_BASE_URL` 填 | 8.5 要开的端口 |
|---|---|---|---|
| **路径 A(ECS 自管)** | ECS-3 上(:8005) | `http://<ECS-3 公网 IP>:8005` | 80 **和** 8005 |
| **路径 B(EDAS)** | ECS-3 上(:8005) | `http://<ECS-3 公网 IP>:8005`(和 A 相同) | 80 **和** 8005 |
| **路径 C(SAE)** | SAE 上(经公网 CLB) | `http://<SAE 公网 CLB 域名>`(Part 7.7 第 26 步记的,无端口) | 只开 80 |

> 看出来了吗:**A 和 B 在本 Part 完全没有区别**(网关都在 ECS-3 上),只有 C 不同(网关搬去了 SAE)。除了"API 地址"和"开不开 8005",**本 Part 其余每一步对三条路径一字不差**。

**问题②:你的 ECS-3 买的什么镜像?——只影响一件事:8.3 / 8.4 选哪种部署方式**

| ECS-3 的镜像(Part 3.1 买的时候选的) | 走哪节 | 操作风格 |
|---|---|---|
| 镜像市场的 **宝塔面板** 镜像(课程推荐) | **8.3 方式①(宝塔面板)** | 网页点鼠标 |
| 裸 `Ubuntu` / `Alibaba Cloud Linux` | **8.4 方式②(命令行)** | 终端粘命令 |

> ⚠️ **千万别把"方式①②"和"路径 A/B/C"搞混**——它们是两回事!部署方式只看你 ECS-3 装的什么镜像,跟你后端走哪条路径**毫无关系**:走路径 B(EDAS)的同学完全可以用方式②命令行部署前端,走路径 A 的也可以用方式①宝塔。
>
> **为什么推荐方式①(宝塔)？** 这门课的原则是 **"能在阿里云控制台点鼠标生成的，就别让你去敲命令搭"**。宝塔面板把"装 Web 服务器、上传网站、配置站点"做成了网页上的按钮，初学者不必记 `systemctl` / `nginx -t` 这些命令，出错也少。
>
> **为什么不用 OSS？** 阿里云 OSS 静态网站托管确实也是"纯点鼠标"，但它会牵出 Bucket 读写权限、静态页 404 兜底等额外概念。用 ECS-3 + Nginx 托管前端，认知负担最低——所以本课程统一用 ECS-3，不绕 OSS。
>
...
**本 Part 的流程图**(所有人都做 8.1→8.2,然后按问题②分叉,最后合流):

```
8.1 本地构建(按问题①填 API 地址) → 8.2 压成 zip
        │
        ├─ ECS-3 是宝塔镜像 → 8.3 方式①(面板点鼠标) ─┐
        │                                            ├→ 8.5 开端口(按问题①) → 8.6 验证
        └─ ECS-3 是裸镜像   → 8.4 方式②(终端粘命令) ─┘
```

### 8.1 在本地电脑构建前端(并告诉它后端地址)——⚠️ 这里用到问题①

构建时要先设一个环境变量 `VUE_APP_BASE_URL`,这是后端 API 的地址。**这个地址会被永久写进打包后的 JS 文件**,不设就默认 `localhost:8005`(用户浏览器会找自己电脑,失败)。

**先按 8.0 问题①确定你要填的地址**(整个 Part 8 只有这一处因路径而异,填错了 8.6 验证时接口会全挂):

- ✅ 走 **路径 A 或 B** 的同学:填 `http://<ECS-3 公网 IP>:8005`(A 和 B 填的一模一样)
- ✅ 走 **路径 C(SAE)** 的同学:填 `http://<SAE 公网 CLB 域名>`(Part 7.7 第 26 步记下的,**无端口号**,CLB 默认 80)

1. 用 VSCode 终端进项目根目录的 `app-admin/`:
   ```bash
   cd app-admin
   ```
2. 下面命令以 **路径 A / B** 的 `http://x.x.x.x:8005` 为例(把 `x.x.x.x` 换成你 **ECS-3 的公网 IP**);**路径 C** 的同学把整个 `http://x.x.x.x:8005` 换成你的 **SAE 公网 CLB 域名**(无端口号),其余不变。
...
### 8.3 方式①：用宝塔面板部署前端（鼠标，推荐）

> 适用于 Part 3.1 给 ECS-3 选了 **预装宝塔面板镜像** 的同学(8.0 问题②)。全程在网页面板点鼠标，不敲命令。**ECS-3 是裸镜像的同学跳到 8.4,这一节不用做。**(再提醒一次:这和你后端走路径 A/B/C 无关)

> 📌 **关于配图**：宝塔是第三方产品（堡塔公司），它的深层界面（添加站点弹窗、伪静态设置等）在阿里云官方文档里没有截图，宝塔自家文档站也不开放抓取——所以下面部分步骤仍是 📷 占位，**建议老师上课时自己截一套**（版本贴近学生看到的，效果反而最好）。能从阿里云官方教程里拿到的图已经配上。

8. **打开宝塔面板**：ECS-3 实例详情页（或镜像市场该镜像的说明里）会给出 **面板地址、初始账号密码**；浏览器打开、登录，按提示完成首次绑定。

   ![宝塔 Linux 面板登录页](img/baota-login.png)
   > △ 阿里云官方教程截图：宝塔登录页长这样（地址形如 `http://<ECS-3 IP>:8888/xxxx`）

   首次登录可能弹出"**推荐安装套件**"对话框（LNMP / LAMP / JAVA 三选一）——**本项目只需要 Nginx**：选 **LNMP（推荐）** 极速安装即可（其中 MySQL/PHP 用不上但不碍事），或直接关掉弹窗、在「软件商店」单独装 Nginx。

   ![宝塔首次登录的推荐安装套件弹窗](img/baota-suite-dialog.png)
   > △ 阿里云官方教程截图：首次进面板会推荐安装套件，选 LNMP 极速安装
...
### 8.4 方式②：命令行部署前端（进阶，裸镜像同学走这条）

> 适用于 ECS-3 选了裸 `Ubuntu` / `Alibaba Cloud Linux` 的同学(8.0 问题②)。**用了宝塔的同学不用做这一节,直接去 8.5。**(注意:这里的"方式②"不是"路径 B"——走任何路径的同学只要 ECS-3 是裸镜像,都走这一节)
...
### 8.5 在网页 GUI 放开 ECS-3 的端口——⚠️ 这里再次用到问题①

> 如果 Part 5.6 已经加过 80 / 8005,跳过本节。

13. 浏览器 ECS 控制台 → ECS-3 → 安全组 → 入方向 → **手动添加**
14. 按你的路径加规则(授权对象都填 `0.0.0.0/0`):

    | 规则 | 路径 A | 路径 B | 路径 C | 说明 |
    |------|:---:|:---:|:---:|------|
    | TCP `80/80` | ✅ | ✅ | ✅ | Nginx 前端,谁都要开 |
    | TCP `8005/8005` | ✅ | ✅ | ❌ 不开 | coffee-app 网关端口——A/B 的网关在 ECS-3 上所以要开;C 的网关在 SAE 上,ECS-3 没有 8005 这个服务,开了也没用 |
...
**Q：前端能打开但接口全失败**
- F12 → Network 看请求地址，若是 `localhost:8005` → 回 Part 8.1 重新构建，按 8.0 问题①设好 `VUE_APP_BASE_URL`
