 ...
 *
 * ── 演示步骤 ──────────────────────────────────────────────────────────
 *   1. 启动服务，查询一条快递轨迹（数据写入 Redis，TTL=3600s）
 *   2. 在 Nacos 控制台 → 配置管理 → 配置列表 中找到：
 *      Data ID：coffee-expresstrack.properties  Group：DEFAULT_GROUP
 *   3. 添加或修改：cache.ttl.expresstrack=60
 *   4. 保存后，再次查询同一快递轨迹，新写入 Redis 的缓存 TTL 变为 60s
 ...
 * @Setter  Lombok 注解，自动生成所有字段的 setter 方法（setExpresstrackTtl()）
 */
@Getter
@Setter
@Component
...
public class CacheProperties {
     ...
     *   - 优先读取配置项 cache.ttl.expresstrack 的值（来源：Nacos Config 或本地 application.yml）
     *   - 如果该配置项不存在，则使用默认值 3600 秒（1 小时）
     *
